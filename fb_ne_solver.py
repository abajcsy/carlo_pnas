...
def FBNE_quadratic_affine_cost_2player_solver(A,B1,B2,Q1,Q2,R1,R2,T,x0,q1,q2):
    # We assume that the two players have the same dimenssion of control inputs, i.e., m1 = m2
    # cost function for the first player: 1/2*x[:,t]'*Q1*x[:,t] + q1'*x[:,t] + 1/2*u[:,t]'*R1*u[:,t]
    # We assume the dynamics is x[:,t+1] = A*x[;,t] + B1*u1[:,t] + B2*u2[:,t]
    # Sample codes: FBNE_quadratic_affine_cost_2player_solver(np.eye(2), np.ones((2,1)), np.ones((2,1)), np.eye(2), np.eye(2), ...
